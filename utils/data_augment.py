...
class DataAugmentForObjectDetection:
    ...
    def _add_noise(self, img):
        ...
        return random_noise(img, mode='gaussian') * 255

    # 调整亮度
    def _change_light(self, img):
        # random.seed(int(time.time()))
        flag = random.uniform(0.1, 1.9)  # flag>1为调暗,小于1为调亮
    ...
    def data_augment(self, img, bboxes, path1=None, path2=None):
        """
        图像增强
        :param img: 图像array
        :param bboxes: 该图像的所有框坐标
        :return: 增强后的图像，增强后图片对应的box
        """
        ...
        while change_num < 1:  # 默认至少有一种数据增强生效
            ...
            # for input_box, box in zip(input_bboxes, bboxes):
            #     if box[2] > img.shape[1] + 1 or box[3] > img.shape[0] + 1:
            #         show(img, [input_box])
            #         show(img, [box])
            #         raise Exception('Image with annotation error x_max > w or y_max > h', box[2], img.shape[1], box[3],
            #                         img.shape[0], input_box, path1, path2)
            #     if box[0] < -1 or box[1] < -1:
            #         show(img, [input_box])
            #         show(img, [box])
            #         raise Exception('Image with annotation error x_min < 0 or y_min < 0', box[0], box[1], input_box,
            #                         img.shape, path1, path2)
